...
import { useUserTunes, useDeleteTune, useTuneAssignments, useDeleteTuneAssignment, useCreateTune } from "../../../hooks/queries";
import { useAllCars, UserTuneCard, withDefaults } from "../../../components/TuneForm";
...
function MyTunesPage() {
  ...
  const [importStatus, setImportStatus] = useState<"idle" | "imported" | "error">("idle");
  ...
  const handleImportTuneFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const key of ["tires", "gearing", "alignment", "antiRollBars", "springs", "damping", "aero", "differential", "brakes"]) {
      ...
      setImportError(err instanceof Error ? err.message : "Failed to import JSON tune");
  ...
  const filteredAssignments = assignments.filter((a) => selectedCar == null || a.carOrdinal === selectedCar);
  ...
  return (
    <div className="flex-1 overflow-auto p-4 space-y-4 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400">{filteredUserTunes.length}</span>
          </div>
          <p className="text-xs text-app-text-muted">Manage your saved tunes and track assignments</p>
        </div>

        <div className="flex items-center gap-2">
          <input ref={importInputRef} type="file" accept=".json,application/json" onChange={handleImportTuneFile} className="hidden" />
          <Button variant="app-outline" size="app-sm" onClick={() => importInputRef.current?.click()} disabled={createTuneMut.isPending}>
            {createTuneMut.isPending ? "Importing..." : importStatus === "imported" ? "Imported" : "Import"}
          </Button>
          ...
          <Link to="/fm23/tunes/catalog" className="text-xs px-3 py-1.5 rounded border border-app-border text-app-text-secondary hover:text-app-text transition-colors no-underline">
            Catalog
          </Link>
      ...
      {importStatus === "error" && importError && <div className="text-xs text-red-400">Import failed: {importError}</div>}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-app-text-muted text-sm">
            ...
            <p className="mt-1">
              ...
              <Link to="/fm23/tunes/catalog" className="text-app-accent hover:underline">
      ...
      {filteredAssignments.length > 0 && (
        <div className="pt-4 border-t border-app-border">
          <h3 className="text-xs font-semibold uppercase tracking-wider text-app-text-muted mb-2">Active Tune Assignments</h3>
