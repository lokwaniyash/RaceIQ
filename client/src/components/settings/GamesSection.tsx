...
export function GamesSection() {
  ...
  function toggle(gameId: string) {
    const next = hiddenGames.includes(gameId)
      ? hiddenGames.filter((id) => id !== gameId)
      : [...hiddenGames, gameId];
  ...
  return (
    <section>
      ...
      <p className="text-sm text-app-text-muted mb-4">
        Choose which games appear in the top navigation and on the home page.
      </p>
      <div className="space-y-2 max-w-sm">
        {games.map((game) => {
          ...
          return (
            ...
            >
              <span className={`text-sm font-medium ${visible ? "text-app-text" : "text-app-text-muted"}`}>
                {game.displayName}
              </span>
              {/* Toggle pill */}
              <span
                className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors ${
                  visible ? "bg-app-accent" : "bg-app-border"
                }`}
              >
                <span
