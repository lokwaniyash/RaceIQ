...
const IMPERIAL_MARKERS = /\b(mph|psi|lb\/in|lb\b|°\s*F|ft\b|feet)\b/i;
const METRIC_MARKERS = /\b(km\/h|kph|bar\b|N\/mm|°\s*C|\bmeters?\b)\b/i;
...
export const unitConsistencyScorer = createScorer({
  id: "unit-consistency",
  description: "Output respects fixture unit system (no metric/imperial bleed-through)",
})
  .generateScore(({ run }) => {
  ...
  .generateReason(({ run }) => {
    ...
    const text = typeof run.output === "string" ? run.output : JSON.stringify(run.output);
    const wrong = units === "metric"
      ? text.match(IMPERIAL_MARKERS)?.[0]
      : text.match(METRIC_MARKERS)?.[0];
    return wrong ? `leaked ${units === "metric" ? "imperial" : "metric"} unit: "${wrong}"` : "clean";
  });
